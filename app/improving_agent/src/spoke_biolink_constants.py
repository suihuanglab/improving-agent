...
PREDICATES = {
    ...
    BIOLINK_ENTITY_DISEASE: {
        BIOLINK_ENTITY_DISEASE: {
            ...
            BIOLINK_ASSOCIATION_PART_OF: SPOKE_EDGE_TYPE_ISA_DiD,
...
for biolink_subject, biolink_object_map in PREDICATES.items():
    for biolink_object, predicates_map in biolink_object_map.items():
        for predicate, spoke_edge_type in predicates_map.items():
            if isinstance(spoke_edge_type, list):
                for spoke_edge in spoke_edge_type:
                    BIOLINK_SPOKE_EDGE_MAPPINGS[predicate].append(spoke_edge)
            else:
                BIOLINK_SPOKE_EDGE_MAPPINGS[predicate].append(spoke_edge_type)
