...
from improving_agent.exceptions import AmbiguousPredicateMappingError, MissingComponentError
...
def _disambiguate_edge_type(predicate, qedge, query_graph):
    ...
    if not objects_map:
        raise NotImplemented501(f'Could not find any supported predicates for subject category: {subject_node.category[0]}')
    ...
    if qedge.predicate:
        for predicate in qedge.predicate:
            ...
            if not spoke_edge_mappings:
                raise NotImplemented501(f'imProving Agent does not currently accept predicates of type {predicate}')
